...
async def process_balance_change(message: Message, state: FSMContext):
    ...
    if target_id in shop_users:
        ...
        try:
            await bot.send_message(target_id, f"✅ Ваш баланс пополнен на {new_balance} RUB")
...
async def give_personal_promo(callback: CallbackQuery, state: FSMContext):
    target_id = callback.data.split("_")[3] # give_personal_promo_ID
...
async def process_broadcast(message: Message, state: FSMContext):
    ...
    for mammoth_id in mammoths:
        try:
            await bot.send_message(chat_id=mammoth_id, text=broadcast_text)
            success += 1
