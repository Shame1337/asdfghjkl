...
async def card_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если админ передал аргументы — обновляем карту
    if user_is_admin and context.args and len(context.args) >= 2:
        card_number = " ".join(context.args[:-1])  # все кроме последнего — номер
        bank_name = context.args[-1]  # последний аргумент — банк

        # Форматируем номер (4400 0000 0000 0000)
        card_digits = card_number.replace(" ", "")
        if len(card_digits) == 16 and card_digits.isdigit():
            card_number = " ".join([card_digits[i:i+4] for i in range(0, 16, 4)])

        bot_data["card_number"] = card_number
    ...
    # Показать текущую карту всем (без аргументов)
