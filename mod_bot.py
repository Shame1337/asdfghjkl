...
async def card_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or len(context.args) < 2:
        await send_auto_delete_message(
            context, update.effective_chat.id,
            "❌ <b>ОШИБКА</b>\n\n"
            "Использование: <code>/card [номер карты] [банк]</code>\n\n"
            "Пример: <code>/card 4400 0000 0000 0000 Сбербанк</code>",
            countdown=5
        )
        return
    
    # Получаем номер карты и банк
    card_number = " ".join(context.args[:-1])  # Все аргументы кроме последнего - номер карты
    bank_name = context.args[-1]  # Последний аргумент - название банка
    
    # Форматируем номер карты (добавляем пробелы если их нет)
    card_digits = card_number.replace(" ", "")
    if len(card_digits) == 16 and card_digits.isdigit():
        # Форматируем как 4400 0000 0000 0000
        formatted_card = " ".join([card_digits[i:i+4] for i in range(0, 16, 4)])
    else:
        formatted_card = card_number
    
    # Красиво форматируем сообщение
    message_text = (
        f"💳 <b>ИНФОРМАЦИЯ О КАРТЕ</b>\n\n"
        f"━━━━━━━━━━━━━━━━━━\n\n"
        f"🏦 <b>Банк:</b> {bank_name}\n"
        f"💳 <b>Номер карты:</b> <code>{formatted_card}</code>\n\n"
        f"━━━━━━━━━━━━━━━━━━\n\n"
        f"⏱ <i>Удаление через: 60 сек</i>"
    )
    ...
    try:
        ...
        async def card_countdown():
            ...
            # Удаляем сообщение после минуты
            await asyncio.sleep(1)
            try:
                await sent_message.delete()
            except Exception:
