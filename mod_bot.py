...
async def periodic_broadcast(app: Application):
    """Отправляет сообщения воркерам каждые 15 минут"""
    while True:
        try:
            ...
            if not bot_data["broadcast_chats"]:
                continue
            
            # Выбираем случайное сообщение
            message_text = random.choice(WORKER_MESSAGES)
            
            # Отправляем сообщение во все сохраненные чаты
            for chat_id in bot_data["broadcast_chats"]:
                try:
                    ...
                    # Если чат недоступен, удаляем его из списка
                    print(f"Ошибка отправки в чат {chat_id}: {e}")
                    if chat_id in bot_data["broadcast_chats"]:
                        bot_data["broadcast_chats"].remove(chat_id)
                        save_data()
        except Exception as e:
            print(f"Ошибка в periodic_broadcast: {e}")
...
def main():
    ...
    # Запуск периодической рассылки сообщений
    async def post_init(app: Application):
        """Инициализация после запуска бота"""
        asyncio.create_task(periodic_broadcast(app))
