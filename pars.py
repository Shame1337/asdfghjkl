...
if not client.is_user_authorized():
    ...
    try:
        ...
        if 'SessionPasswordNeededError' in str(type(e).__name__):
            password = input(gr+'[+] Введите пароль 2FA: '+re)
...
for chat in chats:
    try:
        # Include both megagroups and channels
        if chat.megagroup == True or chat.broadcast == True:
            groups.append(chat)
    except:
        continue
 
print(gr+'[+] Выберите группу/канал для парсинга участников :'+re)
i=0
for g in groups:
    chat_type = "Канал" if getattr(g, 'broadcast', False) else "Группа"
    print(gr+'['+cy+str(i)+gr+']'+cy+f' - {g.title} ({chat_type})')
    i+=1
